...
function tailorPageToJob(jobId) {
  var boostVcJobId = "1f414f6e-01fe-417b-89f9-81b503423f00";
  if (!jobId) {
    jobId = boostVcJobId;
  }
  job = job_info[jobId];
  ...
  if (job["industries"].length > 0) {
    ...
    if (industry in industry_reasons) {
      ...
      $(".industry_reason").text(industryReason);
      setCookie("industry_reason", industryReason);
    }
  }
  // enable contact for boostvc
  if (jobId === boostVcJobId) {
    $("#contact_disabled").hide();
    $("#contact_enabled").show();
  } else {
    $("#contact_disabled").show();
    $("#contact_enabled").hide();
  }
}
...
function main() {
  ...
  if (id["id"] in job_info) {
    ...
    if (jobId != null) {
      ...
      // default to BoostVC
      tailorPageToJob("1f414f6e-01fe-417b-89f9-81b503423f00");
    }
  }
}
$(function () {
